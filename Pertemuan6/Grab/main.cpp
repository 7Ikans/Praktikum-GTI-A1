...
void Balok (float panjang, float lebar, float tinggi) {
    glPushMatrix();
    float p = panjang/2; float l = lebar/2; float t = tinggi/2;
    //depan
    glBegin(GL_QUADS);
    glVertex3f(-p,0,l); glVertex3f (p,0,l);
    glVertex3f (p,-t*2,l); glVertex3f(-p,-t*2,l);
    glEnd();
    // belakang
    glBegin(GL_QUADS);
    glVertex3f(-p,0,-l); glVertex3f(p,0,-l);
...
void Object() {
    //Meja
    glPushMatrix(); glColor3f(0.1,0.1,0.2);
    glTranslatef (0,3,0); Balok (5,5,3); glPopMatrix();
    // Kotak pink
    glPushMatrix(); perubahKotak(); glColor3f(0.8,0.3,0.3);
    glTranslatef (posXKotak, posYKotak, 0); Balok (3,3,3); glPopMatrix();
    
    glPushMatrix(); pengubahTangan();
    // objek yang dirubah
    glColor3f(0.3,0.3,0.8); glTranslatef (posXBadan, posYBadan,0);
    Balok (3,3,7); //badan
    
    // tangan kiri
    glPushMatrix(); glColor3f(0.2,0.5,0.2); glTranslatef (0,-2,2.5);
    glRotatef (rotTangan1,1,0,0); glRotatef (rotTangan2,0,0,1);
    Balok (2,2,4); glPopMatrix();
...
void display() {
    ...
    Object(); glPopMatrix(); glFlush(); glutSwapBuffers();
}
...
int main(int argc, char **argv) {
    ...
    glutIdleFunc (display); glutReshapeFunc (Reshape);
    lighting(); init(); glutMainLoop(); return(0);
}
